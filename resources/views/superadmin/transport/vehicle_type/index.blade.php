            ...
            <div class="col-md-12">
                <div class="card card-plain">
                    <div class="card-header card-header-primary">
                        <a href="{{route('transport.vehicle-type.create')}}" class="btn btn-info" style="float:right;"><i class="material-icons">add</i> Add New</a>
                        <h4 class="card-title mt-0" style="padding-top: 7px;margin-left: 5px;font-size: 17px;">Vehicle Type List</h4>
                    </div>
                    <div class="card-body">
...
<script>
    $(function() {
        $("#example1").DataTable({
            ...
            serverSide: true,
            order: [0, "desc"],
            ajax: "{{ route('transport.vehicle-type.list') }}",
            columns: [{
                    data: 'id',
                    name: 'id'
                },
...
<script>
    function changeStatus(id) {
        ...
        if ($(mainParent).find('input.approve' + id).is(':checked')) {
            $.ajax({
                url: '{{route("transport.vehicle-type.change.status")}}',
                method: 'POST',
                data: {
                    'id': id,
                    '_token': '{{csrf_token()}}'
