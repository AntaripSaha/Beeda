...
@section('main_content')
    <div class="content">
        <div class="container-fluid">
                ...
                <div class="col-md-8">
                    <div class="card">
                        <div class="card-header card-header-primary">
                            <h4 class="card-title">Assign Vehicle Icon</h4>
                        </div>
                        <div class="card-body">
                            <form action="{{route('transport.vehicle-sub-type.assign.vehicle.icon.submit')}}" method="post" enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="id" value="{{$vehicleSubTypeId}}"/>
                                <div class="row">
                                    <div class="col-md-4">
                                        @foreach($vehicleIcons as $icon)
                                            <label><input type="radio" value="{{$icon->id}}" name="icon" @if($assignedVehicleIcon && $assignedVehicleIcon->vehicle_icon_id == $icon->id) checked @endif/>&nbsp;&nbsp;{{$icon->name}}</label>&nbsp;
                                        @endforeach
                                    </div>
                                </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <a href="{{url()->previous()}}" class="btn btn-success pull-left">Back</a>
                </div>
            </div>
        </div>
    </div>
