            ...
            <div class="col-md-12">
              <div class="card card-plain">
                <div class="card-header card-header-primary">
                  <a href="{{route('subscriptions.driver')}}" class="btn btn-info" style="float:right;"><i class="material-icons">toc</i> Back Subscription List</a>
                  <h4 class="card-title mt-0" style="padding-top: 7px;margin-left: 5px;font-size: 17px;">
                    {{ $user->name }} Subscription List
                  </h4>
                </div>
                ...
                <div class="card-deck">
                  <div class="card btn btn-info">
                    <div class="card-body">
                      <h5 class="card-title" style="color:white">Ride Subscription</h5>
                      <h3 class="card-text"><b>${{ $ride_amount }}</b></h3>
                    </div>
                  </div>
                  <div class="card  btn btn-primary">
                    <div class="card-body">
                      <h5 class="card-title"  style="color:white">Courier Subscription</h5>
                      <h3 class="card-text"><b>${{ $courier_amount }}</b></h3>
                    </div>
                  </div>
                  <div class="card  btn btn-success">
                    <div class="card-body">
                      <h5 class="card-title"  style="color:white">Total Revenue Generated</h5>
                      <h3 class="card-text"><b>${{ $total_revenue }}</b></h3>
                    </div>
                  </div>
                </div>
...
<script>
  $(function () {
    ...
    $("#example1").DataTable({
      ...
      serverSide: true,
      order: [0, "asc"],
      ajax: "{{ route('subscriptions.driver.details', ['user_id' => $user->id]) }}",
      columns: [
          {data: function() { return i++}, name: 'id'},
...
function publish(id)
{
  ...
  if($(mainParent).find('input.publish'+id).is(':checked')) {
    $.ajax({
            url: '{{route("subscriptions.publish")}}',
            method: 'POST',
            data: {'subscription_id': id, '_token': '{{csrf_token()}}'},
...
{
  if(confirm("Are you sure to delete the subscription?"))
  {
