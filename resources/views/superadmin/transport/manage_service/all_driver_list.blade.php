      ...
      <div class="col-md-12">
        <div class="card card-plain">
          <div class="card-header card-header-primary">
            <h4 class="card-title mt-0">All Driver List</h4>
            <!-- <p class="card-category">(Store of services)</p> -->
          </div>
          <div class="card-body">
...
<script>
  $(function() {
    $("#example1").DataTable({
      ...
      serverSide: true,
      order: [0, "desc"],
      ajax: "{{ route('transport.service.all-drivers') }}",
      columns: [{
          "data": 'DT_RowIndex'
        },
        {
...
<script>
  function approve(id) {
    ...
    if ($(mainParent).find('input.approve' + id).is(':checked')) {
      $.ajax({
        url: '{{route("driver.approve")}}',
        method: 'POST',
        data: {
          'transport_driver_id': id,
          '_token': '{{csrf_token()}}'
        },
