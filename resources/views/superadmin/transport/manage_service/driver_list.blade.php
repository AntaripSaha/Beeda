      ...
      <div class="col-md-12">
        <div class="card card-plain">
          <div class="card-header card-header-primary">
            <h4 class="card-title mt-0" style="margin-left: 4px;">{{$service->name}} Transport List</h4>
          </div>
          <div class="card-body">
            <table id="example1" class="table table-bordered table-striped">
              <thead>
                <tr>
                  ...
                  <th>Contact No.</th>
                  <th>Ratings</th>
                  @if($service->category_type==5)
                  <th>Curier Deliveries</th>
                  @else
                  <th>Rides</th>
                  @endif
                  <th>Documents</th>
...
<script>
  $(function() {
    $("#example1").DataTable({
      ...
      serverSide: true,
      order: [0, "desc"],
      ajax: "{{ route('transport.service.driver.list', ['id' => $service_id]) }}",
      columns: [{
          "data": 'DT_RowIndex'
        },
        {
...
<script>
  function approve(user_id, service_id) {

    var mainParent = $('.approve' + user_id).parent('.toggle-btn');
    if ($(mainParent).find('input.approve' + user_id).is(':checked')) {
      $.ajax({
        url: '{{route("service.driver.approve")}}',
        method: 'POST',
        data: {
          'user_id': user_id,
        ...
        success: function(data) {
          ...
          $('.approve' + user_id).attr("checked", "checked");
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          $('.feature' + user_id).removeAttr('checked');
          customShowNotification('top', 'right', "Status changed successfully");
        }
      });
    }
