      ...
      <div class="col-md-12">
        <div class="card card-plain">
          <div class="card-header card-header-primary">
            <h4 class="card-title mt-0" style="padding-top: 7px;margin-left: 5px;font-size: 17px;">Transactions</h4>
          </div>
          <div class="card-body">
...
<script>
  $(function() {
    $("#example1").DataTable({
      ...
      serverSide: true,
      order: [0, "desc"],
      ajax: "{{ route('wallet.bank-transfer') }}",
      columns: [{
          data: 'id',
          name: 'id'
        },
  ...
  function changeStatus(id) {
    Swal.fire({
      ...
      if (result.value) {
        ...
        console.log(status);
        $.ajax({
          url: "{{route('wallet.bank-transfer-status')}}",
          type: "POST",
          data: {
            id: id,
            status: status,
