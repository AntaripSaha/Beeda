...
@section('page_title')
Transactions List of {{$pinRequest->user->name}}
@endsection
      ...
      <div class="col-md-12">
        <div class="card card-plain">
          <div class="card-header card-header-primary">
            <h4 class="card-title mt-0" style="padding-top: 7px;margin-left: 5px;font-size: 17px;">{{$pinRequest->user->name}}'s Transaction History</h4>
          </div>
          <div class="card-body">
            <h3>Current Balance: ${{$pinRequest->user->balance}}</h3> 
            @if($pinRequest->status=='Pending')
            <a href="../approve-pin-request/{{$pinRequest->id}}" class="btn btn-success">Approve Pin Reset Request</a>
            @endif
...
<script>
  $(function() {
    $("#example1").DataTable({
      ...
      serverSide: true,
      order: [0, "desc"],
      ajax: "{{ route('wallet.details',$pinRequest->id) }}",
      columns: [{
          data: 'id',
          name: 'id'
        },
