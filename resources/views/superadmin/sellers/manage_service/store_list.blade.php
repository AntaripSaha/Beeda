            ...
            <div class="col-md-12">
              <div class="card card-plain">
                <div class="card-header card-header-primary">
                  <h4 class="card-title mt-0" style="margin-left: 4px;">{{$service->name}} Store List</h4>
                </div>
                <div class="card-body">
...
<script>
  $(function () {  
    $("#example1").DataTable({
      ...
      serverSide: true,
      order: [0, "desc"],
      ajax: "{{ route('service.store.list', ['id' => $service_id]) }}",
      columns: [
          {data: 'id', name: 'id'},
          {data: 'logo', name: 'logo'},
          {data: 'name', name: 'name'},
...
function approve(id)
{
  ...
  if($(mainParent).find('input.approve'+id).is(':checked')) {
    $.ajax({
            url: '{{route("service.store.approve")}}',
            method: 'POST',
            data: {'seller_service_id': id, '_token': '{{csrf_token()}}'},
...
function feature(id)
{
  ...
  if($(mainParent).find('input.feature'+id).is(':checked')) {
    $.ajax({
            url: '{{route("service.store.feature")}}',
            method: 'POST',
            data: {'shop_id': id, '_token': '{{csrf_token()}}'},
            success: function(data){
              $(mainParent).addClass('active');
              $('.feature'+id).attr("checked", "checked");
              customShowNotification('top', 'right', "Featured status changed successfully");
            }
        });
  } else {
    $.ajax({
