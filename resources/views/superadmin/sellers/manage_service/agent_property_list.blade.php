            ...
            <div class="col-md-12">
              <div class="card card-plain">
                <div class="card-header card-header-primary">
                  <h4 class="card-title mt-0" style="margin-left: 4px;">Agent Property List</h4>
                </div>
                <div class="card-body">
...
<script>
  $(function () {  
    $("#example1").DataTable({
      ...
      serverSide: true,
      order: [0, "desc"],
      ajax: "{{ route('service.agent.property.list', ['agent_id' => $agent_id]) }}",
      columns: [
          {data: 'id', name: 'id'},
...
function feature(id)
{
  ...
  if($(mainParent).find('input.feature'+id).is(':checked')) {
    $.ajax({
            url: '{{route("agent.property.feature")}}',
            method: 'POST',
            data: {'property_id': id, '_token': '{{csrf_token()}}'},
...
{
  var mainParent = $('.recommend'+id).parent('.toggle-btn');
  if($(mainParent).find('input.recommend'+id).is(':checked')) {
    $.ajax({
            url: '{{route("agent.property.recommend")}}',
            method: 'POST',
            data: {'property_id': id, '_token': '{{csrf_token()}}'},
            success: function(data){
              $(mainParent).addClass('active');
              $('.recommend'+id).attr("checked", "checked");
              customShowNotification('top', 'right', "Status changed successfully");
            }
        });
  } else {
    $.ajax({
            ...
            success: function(data){
              $(mainParent).removeClass('active');
              $('.recommend'+id).removeAttr('checked');
              customShowNotification('top', 'right', "Status changed successfully");
            }
        });
  }
