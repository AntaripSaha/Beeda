            ...
            <div class="col-md-12">
              <div class="card card-plain">
                <div class="card-header card-header-primary">
                  <h4 class="card-title mt-0" style="padding-top: 7px;margin-left: 5px;font-size: 17px;">Seller List</h4>
                </div>
                <div class="card-body">
...
function approve(id)
{
  ...
  if($(mainParent).find('input.approve'+id).is(':checked')) {
    $.ajax({
            url: '{{route("seller.approve")}}',
            method: 'POST',
            data: {'seller_id': id, '_token': '{{csrf_token()}}'},
            success: function(data){
              $(mainParent).addClass('active');
              $('.approve'+id).attr("checked", "checked");
              customShowNotification('top', 'right', "Seller status changed successfully");
            }
        });
  } else {
    $.ajax({
