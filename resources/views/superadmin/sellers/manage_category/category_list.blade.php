      ...
      <div class="col-md-12">
        <div class="card card-plain">
          <div class="card-header card-header-primary">
            <a href="{{route('category.add')}}" class="btn btn-info" style="float:right;"><i class="material-icons">add</i> Add New</a>
            <h4 class="card-title mt-0" style="padding-top: 7px;margin-left: 5px;font-size: 17px;">Category List</h4>
          </div>
          <div class="card-body">
    ...
    if ($(mainParent).find('input.feature' + id).is(':checked')) {
      $.ajax({
        ...
        success: function(data) {
          ...
          customShowNotification('top', 'right', "Category featured successfully");
        }
      });
    } else {
      $.ajax({
        url: '{{route("category.unfeature")}}',
        method: 'POST',
        data: {
          'category_id': id,
          '_token': '{{csrf_token()}}'
        ...
        success: function(data) {
          ...
          customShowNotification('top', 'right', "Category unfeatured successfully");
        }
      });
    }
  }
    ...
    if ($(mainParent).find('input.tab' + id).is(':checked')) {
      $.ajax({
        ...
        success: function(data) {
          $(mainParent).addClass('active');
          $('.feature' + id).attr("checked", "checked");
          customShowNotification('top', 'right', "Category tabed successfully");
        }
      });
    } else {
      $.ajax({
        url: '{{route("category.untab")}}',
        method: 'POST',
        data: {
          'category_id': id,
          '_token': '{{csrf_token()}}'
        ...
        success: function(data) {
          ...
          customShowNotification('top', 'right', "Category untabed successfully");
        }
      });
    }
  }

  function MobileTop(id) {
    var mainParent = $('.mobileTop' + id).parent('.toggle-btn');
    if ($(mainParent).find('input.mobileTop' + id).is(':checked')) {
      $.ajax({
        url: '{{route("category.mobileTop")}}',
        method: 'POST',
        data: {
          'category_id': id,
          '_token': '{{csrf_token()}}'
        ...
        success: function(data) {
          $(mainParent).addClass('active');
          $('.feature' + id).attr("checked", "checked");
          customShowNotification('top', 'right', "Category mobile top successfully");
        }
      });
    } else {
      $.ajax({
        url: '{{route("category.unmobileTop")}}',
        method: 'POST',
        data: {
          'category_id': id,
          '_token': '{{csrf_token()}}'
        ...
        success: function(data) {
          $(mainParent).removeClass('active');
          $('.mobileTop' + id).removeAttr('checked');
          customShowNotification('top', 'right', "Category mobile top successfully");
        }
      });
    }
