            ...
            <div class="col-md-12">
              <div class="card card-plain">
                <div class="card-header card-header-primary">
                  <h4 class="card-title mt-0" style="padding-top: 7px;margin-left: 5px;font-size: 17px;">Login Activity</h4>
                </div>
                <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tr>
                    <th>ID</th>
                    <th>Device Name | <span style="background-color:#adf0cf;padding:4px;border-radius:1px;font-size:11px;font-weight:bold">Active</span>
                    <span style="background-color:#f2c2bb;padding:4px;border-radius:1px;font-size:11px;font-weight:bold">Inactive</span>
                  </th>
                    <th>Device Type</th>
                    <th>Login Time</th>
                  </tr>
              ...
              <a href="{{route('superadmin.customer.details',['id'=>$user->id])}}" class="btn btn-dark">User Details</a>
            </div>
          </div>
        </div>
      </div>
...
<script>
  $(function () {
    $("#example1").DataTable({
      ...
      serverSide: true,
      order: [0, "desc"],
      ajax: "{{ route('superadmin.customer.login-activity',['id'=> $user->id]) }}",
      columns: [
          {data: 'id', name: 'id'},
