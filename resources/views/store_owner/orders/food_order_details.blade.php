                                                        ...
                                                        <table class="table table-striped">
                                                            <thead>
                                                                <tr class="line">
                                                                    ...
                                                                    <td class="text-center"><strong>ADDONS</strong></td>
                                                            ...
                                                            <tbody>
                                                                @php $sl = 0; $subtotal = 0; $taxes = 0; $shipping_cost = 0; $addon_price = 0; $coupon_discount = 0;@endphp
                                                                ...
                                                                <tr>
                                                                    <td>{{++$sl}}</td>
                                                                    <td style="text-align:center;"><img src="{{assetUrl().$details->product->thumbnail_image->file_name}}" style="height:65px;" /></td>
                                                                    <td class="text-center">{{$details->product->name}}</td>
                                                                    <td class="text-center">
                                                                        @foreach($details->order_addons as $addons)
                                                                            @php $addon_price += $addons->addon->price; @endphp
                                                                            {{$addons->addon->name}}: {{number_format($addons->addon->price, 2)}}<br>
                                                                        @endforeach
                                                                    </td>
                                                                    <td class="text-center">{{$details->price}}</td>
                                                                    <td class="text-center">{{$details->quantity}}</td>
                                                                    <td class="text-right">{{number_format(($details->price + $addon_price), 2)}}</td>
                                                                </tr>
                                                                @endforeach
                                                                <tr>
                                                                    <td colspan="5"></td>
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    @php $grand_total = ($subtotal + $addon_price + $taxes + $shipping_cost) - $coupon_discount; @endphp
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="ticket" id="ticket" style="margin: 0 auto;text-align: center;display:none;">
                    ...
                    <!-- <img class="pos_logo" src="{{assetUrl().$order->order_details[0]->product->shop->logo_img->file_name}}" style="height:150px;border-radius:50%;margin:0 auto;" alt="Logo"> -->
                    ...
                    <table class="pos_table" style="margin: 0 auto;">
                        ...
                        <tbody>
                            @php $sl = 0; $addon_price = 0; @endphp
                            @foreach($order->order_details as $details)
                            <tr>
                                <td class="sl">{{++$sl}}</td>
                                <td class="item">
                                    {{$details->product->name}}
                                    @if(count($details->order_addons)>0)
                                    @foreach($details->order_addons as $addons)
                                        @php $addon_price += $addons->addon->price; @endphp
                                        <br>{{$addons->addon->name}}(Addon): {{number_format($addons->addon->price, 2)}}
                                    @endforeach
                                    @endif
                                </td>
                                <td class="quantity">{{$details->quantity}}</td>
                                <td class="price">{{number_format(($details->price), 2)}}</td>
                                <td class="subtotal">{{number_format(($details->price + $addon_price), 2)}}</td>
                            </tr>
                            @endforeach    
                            <tr>
