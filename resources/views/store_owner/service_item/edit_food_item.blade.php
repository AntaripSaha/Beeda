...
<div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Edit Food Product</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item"><a href="#">Edit Food Product</a></li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <input type="hidden" name="vegetarian" value="{{$product->veg}}" id="veg" />
              ...
              <!-- /.card-header -->
                ...
                <div class="form-group">
                    @php
                     ...
                     {
                         $add_ids[] = $addon->food_addon_id;
                     }
                    @endphp
                  <label for="description">Addons</label>
                  <select class="addon-basic-multiple form-control" name="addons[]" multiple="multiple">
                    @if($data)
                    @foreach($data->addons as $addon)
                    <option value="{{$addon->id}}" @if(in_array($addon->id, $add_ids)) selected @endif>{{$addon->name}}</option>
                    @endforeach
                    @endif
                  </select>
                </div>
                  ...
                  <div class="form-group">
                    <label for="food_preparation_time">Food Preparation Time (In minuites)</label>
                    @php
                      $preparation_time = $product->food_preparation_time ? explode(' ', $product->food_preparation_time)[0] : null;
                    @endphp
                    <input type="number" min='1' step="any" class="form-control" value="{{$preparation_time}}" name="food_preparation_time" id="food_preparation_time" placeholder="">
                    <span style="color:red;font-size:13px;">{{ $errors->first('food_preparation_time') }}</span>
                  </div>
                </div>
                <!-- /.card-body -->
            </div>
        ...
        <div class="col-4">
              ...
              <div class="card-body">
                ...
                <div class="form-group">
                  ...
                  <div class="shipping_cost" @if($product->shipping_type == 'flat_rate') style="display:;" @else style="display:none;" @endif>
                      ...
                      <input type="text" name="flat_shipping_cost" style="float:right" value="{{$product->shipping_type == 'flat_rate' ? $product->shipping_cost : 0}}" class="form-control col-md-4" />
                  </div>
                </div>
                <div class="form-group">
                  <label for="product_wise_shipping">Product Wise Shipping</label>
                  <div class="toggle-btn @if($product->shipping_type == 'product_wise') active @endif" style="margin:0px;float:right;">
                      <input type="checkbox" name="product_wise_shipping" onchange="config('product_wise')" class="cb-value product_wise_shipping" @if($product->shipping_type == 'product_wise') active @endif/>
                      <span class="round-btn"></span>
                  </div>
                  <div class="product_wise_cost" @if($product->shipping_type == 'product_wise') style="display:;" @else style="display:none;" @endif>
                    @php $product_wise = json_decode($product->shipping_cost);@endphp
                      @foreach($data->cities as $city)
                      @php $city_name = $city->name;@endphp
                      <label for="product_wise_cost">{{$city->name}}</label>
                      <input type="text" name="shipping_cost[{{$city->name}}]" style="float:right" value="{{$product->shipping_type == 'product_wise' ? $product_wise->$city_name : $city->cost}}" class="form-control col-md-4" />
                      <br>
                      <br>
                      @endforeach
                  </div>
                ...
                <div class="form-group">
                  ...
                  <div class="toggle-btn @if($product->is_quantity_multiplied == 1) active @endif" style="margin:0px;float:right;">
                      <input type="checkbox" name="product_quantity_multiply" onchange="productQuantityMultiply()" class="cb-value product_quantity_multiply" @if($product->is_quantity_multiplied == 1) checked @endif/>
                      <span class="round-btn"></span>
                  </div>
                </div>
              </div>
          ...
          <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Food Type</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <div class="form-group">
                  <label for="veg">Vegetarian</label>
                  <div class="toggle-btn @if($product->veg) active @endif" style="margin:0px;float:right;">
                    <input type="checkbox" name="veg" onchange="vegStatus()" class="cb-value veg" @if($product->veg) checked @endif/>
                    <span class="round-btn"></span>
                  </div>
                </div>
              </div>
          ...
          <div class="card card-primary">
            <div class="card-header">
              <h3 class="card-title">Product Pickup Service</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
              <label for="pickup">Pickup Status</label>
              <div class="row">
                <div class="col-12">
                  <div class="form-group">
                    <select class="form-control" name="pickup">
                      <option value="0" @if($product->pickup == 0) selected @endif>Default</option>
                      @if($shop->pickup_status)
                      <option value="1" @if($product->pickup == 1) selected @endif>Pickup</option>
                      @endif
                    </select>
